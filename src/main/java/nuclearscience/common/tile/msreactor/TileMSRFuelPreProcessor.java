...
public class TileMSRFuelPreProcessor extends GenericTile {
	...
	public TileMSRFuelPreProcessor(BlockPos pos, BlockState state) {
		...
		addComponent(new ComponentFluidHandlerMulti(this).setTanks(1, 1, new int[] { MAX_TANK_CAPACITY }, new int[] { MAX_TANK_CAPACITY }).setInputDirections(BlockEntityUtils.MachineDirection.RIGHT).setRecipeType(NuclearScienceRecipeInit.MSR_FUEL_PREPROCESSOR_TYPE.get()));
